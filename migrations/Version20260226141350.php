 ...
 */
final class Version20260226141350 extends AbstractMigration
{
    ...
    public function up(Schema $schema): void
    {
        ...
        if (!$this->getSchemaManager()->tablesExist(['campany_contact_contact'])) {
        ...
        if (!$this->getSchemaManager()->tablesExist(['document_rag_chunk'])) {
        ...
        if (!$this->getSchemaManager()->tablesExist(['document_rag_index'])) {
        ...
        $contactTable = $this->getTable('contact');
        if ($contactTable !== null && $this->hasForeignKey($contactTable, 'FK_4C62E6385F59C144')) {
            $this->addSql('ALTER TABLE contact DROP FOREIGN KEY FK_4C62E6385F59C144');
            $contactTable = $this->getTable('contact');
        }
        if ($contactTable !== null && $contactTable->hasIndex('IDX_4C62E6385F59C144')) {
            $this->addSql('DROP INDEX IDX_4C62E6385F59C144 ON contact');
            $contactTable = $this->getTable('contact');
        }
        if ($contactTable !== null && $contactTable->hasColumn('campany_id')) {
            $this->addSql('ALTER TABLE contact DROP campany_id');
        }

        $fundingRequestTable = $this->getTable('funding_request');
        if ($fundingRequestTable !== null && !$fundingRequestTable->hasColumn('comment')) {
            $this->addSql('ALTER TABLE funding_request ADD comment LONGTEXT DEFAULT NULL');
            $fundingRequestTable = $this->getTable('funding_request');
        }
        if ($fundingRequestTable !== null && !$fundingRequestTable->hasColumn('decision')) {
            $this->addSql('ALTER TABLE funding_request ADD decision VARCHAR(255) DEFAULT NULL');
        }
    }
    ...
    public function down(Schema $schema): void
    {
        ...
        if ($contactTable !== null && $contactTable->hasColumn('campany_id') && !$this->hasForeignKey($contactTable, 'FK_4C62E6385F59C144')) {
            $this->addSql('ALTER TABLE contact ADD CONSTRAINT FK_4C62E6385F59C144 FOREIGN KEY (campany_id) REFERENCES campany (id) ON UPDATE NO ACTION ON DELETE NO ACTION');
            $contactTable = $this->getTable('contact');
        }
        if ($contactTable !== null && $contactTable->hasColumn('campany_id') && !$contactTable->hasIndex('IDX_4C62E6385F59C144')) {
            $this->addSql('CREATE INDEX IDX_4C62E6385F59C144 ON contact (campany_id)');
        }

        $fundingRequestTable = $this->getTable('funding_request');
        if ($fundingRequestTable !== null && $fundingRequestTable->hasColumn('comment')) {
            $this->addSql('ALTER TABLE funding_request DROP comment');
            $fundingRequestTable = $this->getTable('funding_request');
        }
        if ($fundingRequestTable !== null && $fundingRequestTable->hasColumn('decision')) {
            $this->addSql('ALTER TABLE funding_request DROP decision');
        }
    }
